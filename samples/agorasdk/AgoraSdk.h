...
class AgoraSdk : public agora::recording::IRecordingEngineEventHandler {
    public:
        AgoraSdk();
        virtual ~AgoraSdk();

        bool createChannel(const string &appid, const string &channelKey, const string &name,  agora::linuxsdk::uid_t uid,
                agora::recording::RecordingConfig &config);
        int setVideoMixLayout();
        virtual bool leaveChannel();
        bool release();
        bool stopped() const;
        void updateMixModeSetting(int width, int height, bool isVideoMix) {
        ...
        void updateStorageDir(const char* dir) { m_storage_dir = dir? dir:"./"; }
