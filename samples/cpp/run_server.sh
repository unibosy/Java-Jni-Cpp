...
case $# in
    3)
        ...
        if [ $1 == "audio" ]; then
           ./recorder_local --recordFileRootDir ./ --channel video  --uid 333 --appliteDir `pwd`/../bin --idle 20 --isAudioOnly 1 --isMixingEnabled 1  $param --decryptionMode aes-128-xts --secret 123 

        elif [ $1 == "video" ]; then 
            ./recorder_local --recordFileRootDir ./ --channel video  --uid 333 --appliteDir `pwd`/../../bin --idle 20 $param #--triggerMode 0   # --getVideoFrame 3 --getAudioFrame 3 
        fi
